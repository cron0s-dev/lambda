...
#define PROMPT_SYM "λ"
#define PROMPT_CHAR '>'
#define PROMPT "%s%s%s %c ", COLOR_MAGENTA, PROMPT_SYM, COLOR_RESET, PROMPT_CHAR
...
void print_expr_result(const char *line, const Expr *expr) {
    ...
    if (expr->type == EXPR_ASSIGN) {
        printf("\n %s%.15g%s -> %s\n\n", COLOR_CYAN, result, COLOR_RESET, expr->assign.name);
    } else {
        printf("\n %s = %s%.15g%s\n\n", line, COLOR_CYAN, result, COLOR_RESET);
    }
}
...
int main(void) {
    ...
    for (;;) {
        ...
        if (expr->type == EXPR_IDENT && strcmp(expr->ident, "clear") == 0) {
            term_clear();
            continue;
        }

        if (expr->type == EXPR_IDENT && strcmp(expr->ident, "exit") == 0) {
            exit(0);
        }
